...
class DioLogInterceptor extends Interceptor {
  ...
  void onRequest(RequestOptions options, RequestInterceptorHandler handler) {
    ...
    if (data != null) {
      ...
      } else if (data is JsonConvert) {
        String body = jsonEncode(data);
        if (body.length > 300) {
          int len = 0;
          while (body.length - len > 300) {
            print("flutter_tag====long_body=====${body.substring(len, len + 300)}");
            len += 300;
          }
          print("flutter_tag====long_body=====${body.substring(len, body.length)}");
        }
        requestStr += "- BODY:\n$body\n";
      } else {
        requestStr += "- BODY:\n${data.toString()}\n";
