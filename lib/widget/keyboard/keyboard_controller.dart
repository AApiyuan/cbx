...
class KeyboardController extends ValueNotifier<TextEditingValue> {
  ...
  set value(TextEditingValue newValue) {
    ...
    //     // 修正由于默认值导致的Bug
    //     composing: TextRange(
    //         start: newValue.composing.start < 0 ? 0 : newValue.composing.start, end: newValue.composing.end < 0 ? 0 : newValue.composing.end),
  ...
  deleteOne() {
    ...
    if (selection.baseOffset != selection.extentOffset) {
      ...
      value = TextEditingValue(
          text: newText, selection: selection.copyWith(baseOffset: selection.baseOffset - 1, extentOffset: selection.baseOffset - 1));
    }
  }

  /// 在光标位置添加文字,一般用于键盘输入
  addText(String insertText) {
    String newText = text == '' ? insertText : (selection.textBefore(text) + insertText + selection.textAfter(text));
    value = TextEditingValue(
        text: newText,
        selection: selection.copyWith(baseOffset: selection.baseOffset + insertText.length, extentOffset: selection.baseOffset + insertText.length));
  }
