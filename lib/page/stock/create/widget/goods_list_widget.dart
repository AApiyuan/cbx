...
class GoodsListWidget extends StatelessWidget {
              ...
              child: GetBuilder<StockController>(
                ...
                builder: (ctl) {
                  return MixSkuEditListView(
                    ...
                    thirdTagName:
                        (ctl.orderStockGoodsType == OrderStockGoodsTypeEnum.NORMAL || ctl.orderStockType == OrderStockTypeEnum.SUBSTANDARD_RECORD)
                            ? "正品库存"
                            : "次品库存",
                    thirdTag:
                        (ctl.orderStockGoodsType == OrderStockGoodsTypeEnum.NORMAL || ctl.orderStockType == OrderStockTypeEnum.SUBSTANDARD_RECORD)
                                                    ...
                                                    builder: (ctl) {
                                                      if (!ctl.negative) {
                                                        if (item.addNum != null && item.addNum != 0) {
                                                          return pText(ctl.staticTitle + item.addNum.toString(),
                                                              ColorConfig.color_1678FF, 34.w,
                                                              fontWeight: FontWeight.w600);
                                                        } else {
                                                          return pText(ctl.staticTitle + item.subtractNum.toString(),
                                                              ColorConfig.color_1678FF, 34.w,
                                                              fontWeight: FontWeight.w600);
                                                        }
                                                      } else {
                                                        return Row(
                                                          children: [
                                                            pText('加' + item.addNum.toString(), ColorConfig
                                                                .color_1678FF, 34.w,
                                                                fontWeight: FontWeight.w600),
                                                            pSizeBoxW(10.w),
                                                            pText('减' + (-item.subtractNum).toString(), ColorConfig
                                                                .color_ff3715, 34.w,
                                                                fontWeight: FontWeight.w600)
                                                          ],
                                                        );
  ...
  showMoreDialog(StockController ctl, item, index, state, goodsName, _moreKey) {
    MoreDialog({
      "叠加": () {
