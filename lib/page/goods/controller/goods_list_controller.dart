...
class GoodsListController extends SuperController {
  ///货品列表类型
  ...
  static const int TYPE_INVENTORY_SUBSTANDARD = 8; //盘点_次品

  /// 专属参数
  static const PARAM_SELECT_ALL = "searchAll";
  ...
  var orderDeliveryId = ArgUtils.getArgument2num(Constant.DELIVERY_ORDER_ID)?.toInt(); // 发货单id
  var customerId =
  ...
  var customerLevel = ArgUtils.getArgument("level");
  var searchAll = ArgUtils.getArgument2bool(PARAM_SELECT_ALL);
  var online = ArgUtils.getArgument2bool(Constant.ONLINE, def: true)!;
  var saleLastPrice = ArgUtils.getArgument2bool(Constant.SALE_LAST_PRICE, def: false)!;
  var returnLastPrice = ArgUtils.getArgument2bool(Constant.RETURN_LAST_PRICE, def: false)!;
  ...
  loadParamToType() {
    switch (goodsType) {
      ...
      case TYPE_INVENTORY_SUBSTANDARD:
        substandardParam();
        break;
    }
  }
  ...
  oweParam() {
    ...
    _oweParam.orderDeliveryId = orderDeliveryId;
    _oweParam.goodsNameSerial = null;
  }
  ...
  Future getGoodsList(bool next) async {
    ...
    String json  = jsonEncode(_page);
    return await GoodsApi.page(_page, online: online, showLoading: true).then((entity) {
  ...
  resetParam(
    ...
    param.deptId = deptId;
    param.status = selectType == SelectType.ALL_GOODS ? null : PullOrOff.ON;
  ...
  /// 是否显示过滤控件（开单部分）
  bool isShowFilter() {
    return goodsType == TYPE_SALE && online && searchText.isEmpty;
  }
  ...
  search(var searchText) {
    ...
    if (this.searchText.isEmpty) {
      ...
      resetParam(
        selectType:
            searchAll ?? false ? SelectType.ALL_GOODS : SelectType.BASIC_STATIC,
        returnCustomerDeliveryNum: true,
      );
    }
    getGoodsList(false);
  }
  ...
  Future getGoodsSku(Goods goods) {
    ...
    return GoodsApi.getSkuList(params, showLoading: true, online: online).then(
