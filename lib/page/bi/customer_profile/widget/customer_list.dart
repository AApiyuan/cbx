...
import 'package:haidai_flutter_module/page/bi/customer_profile/widget/customer_chart.dart';
import 'package:haidai_flutter_module/page/bi/customer_profile/widget/customer_info.dart';
import 'package:haidai_flutter_module/page/bi/customer_profile/widget/customer_owe_statistic.dart';
import 'package:haidai_flutter_module/page/bi/customer_profile/widget/customer_sort_title.dart';
import 'package:haidai_flutter_module/page/bi/customer_profile/widget/dept_remit_static_detail.dart';
import 'package:haidai_flutter_module/page/bi/customer_profile/widget/dept_sale_static_detail.dart';
import 'package:haidai_flutter_module/routes/app_pages.dart';
import 'package:haidai_flutter_module/theme/color.dart';
import 'package:haidai_flutter_module/widget/net_image.dart';
import 'package:haidai_flutter_module/widget/p_common.dart';

import 'customer_static_title.dart';
...
Widget customerList() {
  return GetBuilder<CustomerProfileController>(builder: (ctl) {
    return EasyRefresh.custom(
        ...
        slivers: <Widget>[
          //信息
          SliverToBoxAdapter(child: customerInfo()),
          //欠货
          SliverToBoxAdapter(child: customerOweStatistic()),
          // 柱状图
          SliverToBoxAdapter(child: customerChart()),
          customerStaticTitle(),
          //各种销售统计
                      ...
                      (context, index) {
                        return Container(
                          ...
                          decoration: BoxDecoration(
                              color: Color(ColorConfig.color_393a58),
                              borderRadius: BorderRadius.only(
                                  bottomRight: Radius.circular(index != ctl.classifyRecords.length - 1 ? 0.w : 20.w),
                                  bottomLeft: Radius.circular(index != ctl.classifyRecords.length - 1 ? 0.w : 20.w))),
                          child: Container(
                              padding: EdgeInsets.only(bottom: 20.w),
                              decoration: BoxDecoration(
                                  color: Color(ColorConfig.color_393a58),
                                  border: Border(
                                      bottom: BorderSide(
                                          width: 1.w,
                                          color: index != ctl.classifyRecords.length - 1 ? Color.fromRGBO(255, 255, 255, 0.1) : Colors.transparent))),
                              child: Row(
                                children: [
                                  Container(
                                    width: 70.w,
                                  ...
                                  Expanded(
                                    ...
                                    alignment: Alignment.centerLeft,
                                    child: pText(ctl.classifyRecords[index].goodsClassifyName??"",
                                        ColorConfig.color_ffffff, 24.w),
                                  )),
                                  Visibility(
                                      visible: ctl.viewList == "classify",
                                      child: Container(
                                        width: 150.w,
                                        alignment: Alignment.centerLeft,
                                        child: pText((ctl.classifyRecords[index].normalSaleGoodsNum).toString(), ColorConfig.color_ffffff, 24.w),
                                      )),
                                  Visibility(
                                      visible: ctl.viewList == "classify",
                                      child: Container(
                                        width: 150.w,
                                        alignment: Alignment.centerLeft,
                                        child: pText((ctl.classifyRecords[index].normalSaleTaxAmount! / 100).toString(), ColorConfig.color_ffffff, 24.w),
                                      )),
                                ],
                              )),
                        );
                      },
                      childCount: ctl.classifyRecords.length,
                    ),
                  );
                }
              })
